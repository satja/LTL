...
\begin{equation}
\begin{aligned}
Loc &= \bigcup_{i=1}^{n} \{on_i,\ broken_i, charged_i\},\quad Glob = \{congestion, lowBattery\}, \\
...
\end{split}
\\\begin{split}
\text{perform } c_4 \to \ & (2, \ \{\{on_1, on_2, on_3, on_4, charged_4, broken_5, on_6, on_7, on_8\},\ \emptyset\}, \cup_{i=1}^{8}\{\neg charged_i \; U \;  lowBattery\})
\end{split}
\\\begin{split}
...
\Description{A three-lane corridor with 12 columns. The robot starts at column 1 in the middle lane, services columns 3, 7, and 9, and detours around occupied middle-lane cells at columns 4 and 8 via lane changes.}
\end{center}
\end{figure}
...
Because the locality planner's maximum runtime (1.21s) is far below its 10-second timeout cap, the asymmetric timeout thresholds (10s for locality, 30s for baselines) do not artificially skew the Mann--Whitney rank ordering in favor of locality.
...
Example 3 (three-lane corridor, 57 instances) & 57/57 & 18/57 & 27/57 \\
\bottomrule
\end{tabular}
\end{table}
