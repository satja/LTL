...
To complement the theoretical analysis, Section \ref{eval} reports an empirical comparison against brute-force and automata/progression baselines on systematic instances derived from all three examples in this paper.
...
Our contribution lies in restricting CONFLICT to a polynomially solvable variant using additional assumptions that reduce its generality without sacrificing too much of its practical use. Motivated from various real-world examples concerning planning in what is essentially a 1D or 2D space (roads, warehouses), such as the example in Sect. \ref{ex1}, we assume that most propositions -- all except for a constant number of them -- are localized. Formally, we introduce locality as the mapping 
\begin{equation}
...
\end{split}
\end{multline}
The initial state is the same as in Example \ref{ex1}, extended with battery-related propositions: $lowBattery$ is false and $charged_i$ is false for all $i$.
...
\caption{Example \ref{ex3}: A three-lane production corridor with required service cells at columns 3, 7, and 9 (middle lane), and occupied crossing cells at columns 4 and 8. The highlighted route shows a collision-free detour using lane changes.}\label{f-ex3}
\end{center}
\end{figure}
...
\begin{equation}
\begin{aligned}
Loc = \bigcup_{i=1}^n \{robot_{i,y}, broken_{i,y}, done_{i,y}, human_{i,y}: y=1,\dots,W\},\\
Glob=\{collision\}.
\end{aligned}
\end{equation}
Hence each location $i$ has at most $4W$ local propositions, so $M=4W$ is constant when $W$ is fixed.

Let $R\subseteq \{1,\dots,n\}\times\{1,\dots,W\}$ be the set of stations that must be serviced.
The value set is
\begin{equation}
\Omega=\{G\neg collision\}\ \cup\ \{FG\,done_{i,y}:(i,y)\in R\}.
\end{equation}
Thus, collisions are always forbidden, and every required station must eventually be repaired and remain repaired.
...
\begin{multline}
\gamma^-(m_{i,y},robot_{i,y})=\gamma^+(m_{i,y},robot_{i+1,y})=\top,\\
\gamma^+(m_{i,y},collision)=human_{i+1,y},
\end{multline}
\begin{multline}
\gamma^-(s_{i,y\to y'},robot_{i,y})=\gamma^+(s_{i,y\to y'},robot_{i,y'})=\top,\\
\gamma^+(s_{i,y\to y'},collision)=human_{i,y'},
\end{multline}
\begin{multline}
\gamma^-(rep_{i,y},broken_{i,y})=\gamma^+(rep_{i,y},done_{i,y})=robot_{i,y}\land \neg human_{i,y},\\
...
\end{split}
\end{equation*}
The robot repairs required cells and detours around occupied crossing zones by switching lanes before entering unsafe cells.
...
The benchmark set consists of 190 systematically generated instances with problem size $n\in[6,24]$ and ten instances per size. The generation procedure is deterministic and size-ordered, with nontrivial damaged-location patterns chosen systematically for each size. Family assignment follows a fixed cyclic pattern over variants, yielding 76 instances from the linear traffic-light family (Example 1), 57 from the charging extension (Example 2), and 57 from the two-lane repair corridor (Example 3). This design provides controlled growth in instance size while preserving the locality assumptions studied in Section \ref{aa}, while also testing whether conclusions remain stable across all three modeling variants rather than only one. In the plots, this means that for each fixed $n$ there are exactly ten benchmark instances.

All methods are run with the same maximum search depth (160) and evaluated per instance under fixed runtime limits. We use a stricter limit for the locality planner (10 seconds) and a looser limit for both baselines (30 seconds), so the comparison does not disadvantage the non-local methods. The locality parameter is fixed at $L=3$, matching the locality structure of the benchmark family. A run is counted as successful only if a planner returns a plan within the time limit and that plan is accepted by independent validation. Reported planning times exclude validation time, so runtime values represent planner-only cost.

\subsection{Results}

Table \ref{t:eval} summarizes success counts and runtime statistics on the full 190-instance suite. The locality-based planner solves all 190 instances within the 10-second limit and all 190 outputs are validated. The automata/progression baseline solves 116 instances and times out on 74, with all 116 successful outputs validated. The brute-force baseline solves 97 instances and times out on 93, with all 97 successful outputs validated. Runtime statistics are computed over successful runs only: the locality planner has mean runtime 0.146s, median 0.02s, and maximum 1.21s; the automata/progression baseline has mean 1.95s, median 0.20s, and maximum 22.67s; and the brute-force baseline has mean 4.76s, median 0.51s, and maximum 28.92s.

To test whether the runtime advantage is statistically significant at the distribution level, we apply one-sided Mann--Whitney U tests with alternative hypothesis that locality runtimes are smaller than baseline runtimes. Using per-instance planner times (including timeout-capped values), we obtain $p=9.88\times 10^{-45}$ for locality versus brute force and $p=1.08\times 10^{-28}$ for locality versus automata/progression. These results strongly support the claim that the locality approach is systematically faster on this benchmark regime.

\begin{table}[ht!]
\centering
\caption{Benchmark summary over 190 instances (successful runs only for runtime statistics).}
\label{t:eval}
\begin{tabular}{lccccc}
\toprule
Method & Success & Timeout & Mean (s) & Median (s) & Max (s) \\
\midrule
Locality-based planner & 190/190 & 0 & 0.146 & 0.02 & 1.21 \\
Automata/progression baseline & 116/190 & 74 & 1.95 & 0.20 & 22.67 \\
Brute-force baseline & 97/190 & 93 & 4.76 & 0.51 & 28.92 \\
\bottomrule
\end{tabular}
\end{table}

To make the cross-example effect explicit, Table \ref{t:eval-families} reports success counts separately for the three benchmark families. The locality planner remains complete in each family. The automata/progression baseline is complete on the first and second families, but loses robustness on the two-lane family. The brute-force baseline loses robustness already on the second family and further on larger two-lane instances.

\begin{table}[ht!]
\centering
\caption{Success counts by benchmark family.}
\label{t:eval-families}
\begin{tabular}{lccc}
\toprule
Family & Locality-based & Automata/progression & Brute-force \\
\midrule
Example 1 (linear traffic lights, 76 instances) & 76/76 & 63/76 & 50/76 \\
Example 2 (charging extension, 57 instances) & 57/57 & 35/57 & 20/57 \\
Example 3 (two-lane corridor, 57 instances) & 57/57 & 18/57 & 27/57 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=0.82\textwidth]{figures/benchmark-time-mean-std-vs-n.png}
\caption{Runtime versus instance size $n$ with ten instances per size. Solid lines show mean runtime for each method and dashed side lines show mean $\pm$ one standard deviation.}
\label{f3}
\end{center}
\end{figure}
...
The central empirical observation is that locality-aware state compression dominates across all three benchmark families, not only on the base linear family. Even with the strictest time budget, the locality planner solves every instance and stays well below two seconds on every run. This is important because the three families stress different parts of the model: the first emphasizes local repair propagation with global congestion safety, the second adds charging-related propositions and additional temporal obligations, and the third introduces lane switches and collision-avoidance structure in a narrow two-dimensional geometry.
Figure \ref{f3} provides the requested aggregate view by number of intersections: for each method, the mean runtime grows with $n$, and the dashed bands show the corresponding mean $\pm$ standard deviation. Figure \ref{f3local} complements this by isolating only the locality planner. This view confirms that locality runtime still increases with instance difficulty, but on a much smaller scale than for the baselines.

The baseline behavior is heterogeneous by family, which helps explain aggregate statistics. On Example 1, both baselines remain strong for most sizes, although brute force still times out on larger instances. On Example 2, the automata/progression baseline remains complete but with noticeably higher runtime than the locality method, while brute force times out on most instances. On Example 3, which combines movement, lane switching, and safety constraints in a richer local transition system, both baselines exhibit significant timeout rates, and the automata/progression baseline also loses completeness under the fixed timeout.

These results are consistent with the theory from Section \ref{pa}. The proposed method keeps only a bounded active window of local propositions, together with global propositions and compact temporal-status memory, and shifts this window left-to-right. In domains that satisfy locality assumptions, this sharply limits effective state growth as $n$ increases. The automata/progression baseline, by construction, explores product states that combine planning valuations with residual temporal formulas, so temporal monitoring overhead increases with structural complexity even when locality is present. The brute-force baseline suffers most from direct state-space growth because it does not exploit structural compression beyond local pruning checks.
...
The evaluation in Section \ref{eval} supports this claim by showing strong runtime and robustness advantages on systematic locality-structured benchmarks spanning all three example families.
