...
In the formal problem definition, there is no a priori bound on $k$: satisfiability is defined over all finite plans.
...
breadth-first search. Equivalently, one may use a $0$--$1$ shortest-path algorithm with the
above edge costs.

\begin{Theorem}\label{t:shortest}
If there exists a satisfactory plan, then breadth-first search on $\mathcal{G}$ returns a
satisfactory plan with the minimum number of actions.
\end{Theorem}

\begin{proof}{}
Breadth-first search returns a path from $S_0$ to a final vertex with the minimum number of
edges. As observed above, every such path contains the same number $n-2L$ of Type~2 edges.
Hence a minimum-edge successful path is also a path with the minimum number of Type~1 edges.
Extracting the actions from its Type~1 edges therefore yields a satisfactory plan of minimum
length.
\end{proof}

The computational complexity remains polynomial. In particular, replacing depth-first search by
breadth-first search does not change the asymptotic bound stated above, although breadth-first
search may require more memory in practice.

\subsection{Best-Effort Optimization and Lexicographic Refinement}\label{softopt}

When no plan satisfies all values in $\Omega$, one may instead search for a plan that violates
values of minimum total weight. Let
\[
w:\Omega \to \mathbb{R}_{\geq 0}
\]
be a weight function. We then consider a weighted extension of the substate graph in which a
transition incurs a penalty whenever some value becomes \emph{irrevocably} unsatisfied on that
transition. Informally, local values are settled when their relevant propositions leave the
mutable interval, while global values are settled either when they are violated by the current
truth assignment or in the final substate.

Thus, the optimization problem is formulated as a shortest-path problem on an augmented substate
graph $\mathcal{G}^{\star}$. The underlying substate still has the form $(i,T,\Theta)$, while
the search additionally maintains the bounded status information required to determine which
values have already been satisfied or lost. For an edge $e$ of $\mathcal{G}^{\star}$, let
$\lambda(e)\geq 0$ denote the total weight of values that become irrevocably unsatisfied on
that transition. For a path $\mathcal{P}$, let
\[
\lambda(\mathcal{P})=\sum_{e\in \mathcal{P}} \lambda(e).
\]
A terminal penalty $\tau(S)$ may be added at final vertices to account for values whose truth
...
maximizing the total satisfied weight.

Since all penalties are nonnegative, this optimization can be solved by a shortest-path
algorithm such as Dijkstra's algorithm. However, the graph inherits the layered structure of
the original construction: Type~1 edges stay within the same layer $i$, while Type~2 edges move
from layer $i-1$ to layer $i$. Therefore, the same computation can be implemented more
...
\begin{Theorem}\label{t:softopt}
Under the same locality assumptions as in the previous subsection, and for fixed $M$, $L$, and
$K$, the weighted optimization variant is solvable in polynomial time.
\end{Theorem}

\begin{proof}{}
Under the locality assumptions, each layer contains only polynomially many distinct augmented
substates in the size of the input, with the hidden constant determined by the bounded active
window and the bounded status information needed by the soft objective. The number of layers is
$O(n)$, and all edge penalties are nonnegative. Hence a shortest-path computation on
$\mathcal{G}^{\star}$ is polynomial. The same conclusion holds for the layer-wise
implementation, which performs intra-layer relaxations for Type~1 edges and inter-layer
propagation for Type~2 edges.
\end{proof}

A useful refinement is to minimize the number of actions among all best-effort plans of minimum
penalty. This is obtained by assigning to each edge the lexicographic cost
\[
c^{\star}(e)=
\begin{cases}
(\lambda(e),1), & \text{if $e$ is of Type~1},\\
(\lambda(e),0), & \text{if $e$ is of Type~2},
\end{cases}
\]
and minimizing the total path cost in lexicographic order. The first component minimizes the
total lost value weight, while the second component minimizes the number of actions among all
value-optimal plans.
...
All methods are run with the same maximum search depth (160) and evaluated per instance under equal fixed runtime limits of 15 seconds per planner. The locality parameter is fixed at $L=3$, matching the largest locality width among the benchmark families. Example \ref{ex3} itself already satisfies locality for $L=2$, but we keep a single fixed $L$ across all families for direct comparability. Although the formal framework does not impose a global bound on plan length, this depth cap is used in experiments for fair and controlled comparison, so empirical completeness claims are with respect to that cap. A run is counted as successful only if a planner returns a plan within the time limit and that plan is accepted by independent validation. Reported planning times exclude validation time, so runtime values represent planner-only cost.
...
\begin{enumerate}
    ...
    \item The number and range of (hyper-) parameter settings explored prior to conducting final experiments have been indicated, along with the effort spent on (hyper-) parameter optimisation. [yes]
