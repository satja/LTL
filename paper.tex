...
A benchmark-based empirical evaluation shows that the proposed locality-based method is substantially faster than the considered baselines on structured locality instances.
\end{abstract}
...
To complement the theoretical analysis, Section \ref{eval} reports an empirical comparison against brute-force and automata/progression baselines.
...
Consider a long production line with $n$ columns and $W$ parallel conveyor lanes, where $W$ is a fixed small constant (eg, $W=3$).
The robot can move forward along the line and switch between neighboring lanes. Human workers occasionally cross the lanes, and the robot must avoid collisions.
...
As a concrete instance, let $n=12$, $W=3$, $R=\{(3,2),(7,2),(9,2)\}$, and let the initial robot position be $(1,2)$.
Suppose crossing zones $(4,2)$ and $(8,2)$ are occupied by humans.
...
The benchmark set consists of 57 systematically generated instances with problem size $n\in[6,24]$ and three instances per size. These instances follow the traffic-light locality structure used throughout the paper: actions depend on and modify bounded neighborhoods, while values include global safety constraints and eventual local repair goals. The generation procedure is deterministic and size-ordered, with broken-light configurations chosen to avoid trivial adjacency patterns whenever possible. This design provides controlled growth in instance size while preserving the locality assumptions studied in Section \ref{aa}.
...
Table \ref{t:eval} summarizes success counts and runtime statistics. The locality planner solves all 57 instances, and the automata/progression baseline also solves all 57 instances. The brute-force baseline solves 47 instances and times out on 10. Runtime statistics are computed over successful runs only: the locality planner has mean runtime 0.001s, median 0.00s, and maximum 0.01s; the automata/progression baseline has mean 1.26s, median 0.01s, and maximum 22.89s; and the brute-force baseline has mean 2.90s, median 0.02s, and maximum 27.66s.

\begin{table}[ht!]
\centering
\caption{Benchmark summary over 57 instances.}
\label{t:eval}
\begin{tabular}{lccccc}
\toprule
Method & Success & Timeout & Mean (s) & Median (s) & Max (s) \\
\midrule
Locality-based planner & 57/57 & 0 & 0.001 & 0.00 & 0.01 \\
Automata/progression baseline & 57/57 & 0 & 1.26 & 0.01 & 22.89 \\
Brute-force baseline & 47/57 & 10 & 2.90 & 0.02 & 27.66 \\
\bottomrule
\end{tabular}
\end{table}
...
The central empirical observation is that locality-aware state compression dominates on this benchmark family. Even with the strictest time budget, the locality planner solves every instance and remains in a near-constant time regime across the tested range of $n$. By contrast, the two baselines exhibit heavier tails in runtime, especially on larger instances. The automata/progression baseline remains complete on this set, but its worst-case runtime is orders of magnitude larger than that of the locality planner. This behavior is expected: progression-based search tracks temporal residuals over a larger effective product space and does not directly exploit interval locality in the same explicit way.

The brute-force baseline shows the weakest scalability. Its median successful runtime is small, indicating that easy instances are handled quickly, but the timeout count demonstrates loss of robustness as size increases. This gap between median and timeout behavior is important: median-only summaries would hide the heavy-tail effect, while the success/timeout statistics reveal practical reliability.

These results are consistent with the theory from Section \ref{pa}. The proposed method keeps only a bounded active window of local propositions (plus global propositions and temporal satisfaction memory) and shifts this window left-to-right. In structured domains that satisfy the locality assumptions, this restriction sharply reduces branching and effective state size. The baselines rely on more general temporal-monitoring search and therefore pay a larger per-instance overhead.

At the same time, the interpretation should remain scoped to the evaluated regime. The benchmark family is intentionally locality-structured and therefore aligned with the assumptions of our algorithm. This is a strength for validating the intended use case, but it is not evidence that the same margins will hold on domains with weak locality or different temporal-value distributions. A broader study with additional domain families, parameter sweeps over $L$, and cross-platform timing would provide a stronger estimate of external validity.
...
The evaluation in Section \ref{eval} supports this claim by showing strong runtime and robustness advantages on systematic locality-structured benchmarks.
