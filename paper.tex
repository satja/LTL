...
The benchmark set consists of 57 systematically generated instances with problem size $n\in[6,24]$ and three instances per size. Each size contributes one instance from each example family, so there are 19 instances from the linear traffic-light family in Example 1, 19 instances from the charging extension in Example 2, and 19 instances from the two-lane repair corridor in Example 3. The generation procedure is deterministic and size-ordered, with nontrivial damaged-location patterns chosen systematically for each size. This design provides controlled growth in instance size while preserving the locality assumptions studied in Section \ref{aa}, while also testing whether conclusions remain stable across all three modeling variants rather than only one.
...
Table \ref{t:eval} summarizes success counts and runtime statistics on the full 57-instance suite. The locality-based planner solves all 57 instances within the 10-second limit and all 57 outputs are validated. The automata/progression baseline solves 45 instances and times out on 12, with all 45 successful outputs validated. The brute-force baseline solves 32 instances and times out on 25, with all 32 successful outputs validated. Runtime statistics are computed over successful runs only: the locality planner has mean runtime 0.095s, median 0.01s, and maximum 0.70s; the automata/progression baseline has mean 1.66s, median 0.09s, and maximum 25.87s; and the brute-force baseline has mean 5.24s, median 0.97s, and maximum 29.16s.

\begin{table}[ht!]
\centering
\caption{Benchmark summary over 57 instances (successful runs only for runtime statistics).}
\label{t:eval}
\begin{tabular}{lccccc}
\toprule
Method & Success & Timeout & Mean (s) & Median (s) & Max (s) \\
\midrule
Locality-based planner & 57/57 & 0 & 0.095 & 0.01 & 0.70 \\
Automata/progression baseline & 45/57 & 12 & 1.66 & 0.09 & 25.87 \\
Brute-force baseline & 32/57 & 25 & 5.24 & 0.97 & 29.16 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[ht!]
\centering
\caption{Success counts by benchmark family (19 instances per family).}
\label{t:eval-families}
\begin{tabular}{lccc}
\toprule
Family & Locality-based & Automata/progression & Brute-force \\
\midrule
Example 1 (linear traffic lights) & 19/19 & 19/19 & 15/19 \\
Example 2 (charging extension) & 19/19 & 19/19 & 7/19 \\
Example 3 (two-lane corridor) & 19/19 & 7/19 & 10/19 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=0.48\textwidth]{benchmark-time-vs-n.png}
\includegraphics[width=0.48\textwidth]{benchmark-time-vs-case.png}
\caption{Benchmark plots: runtime versus instance size $n$ (left) and runtime per case (right).}
\label{f3}
\end{center}
\end{figure}
